...
def get_membermatch(request):
    ...
    if request.method == "GET" and request.content_type in ['application/json', 'application/json+fhir']:
        member = {}
        coverage = {}
        requester = {}
        beneficiary = {}
        ...
        if coverage:
            ...
            if 'identifier' in coverage:
                ...
                plan_coverage = f_get(URL + resource, data={}, payload=search_payload)
                ...
                if coverage_count == 0:
                    return JsonResponse({"error": "member id not found"}, status=404)
                if coverage_count > 1:
                    return JsonResponse({"error": "unique member id not found"}, status=404)
